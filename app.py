...
def generate_audio_endpoint():
    ...
    try:
        ...
        # Prompt ajustado para solicitar áudio explicitamente
        prompt = f"Generate audio for the following text with a {style} using voice {voice}: {text}"
        response = model.generate_content(
            prompt,
            generation_config={
                "response_mime_type": "text/plain"  # Usa texto como fallback, esperando que a API processe o áudio internamente
            }
        )

        # Verifica se a resposta contém dados de áudio
        if not response.candidates[0].content.parts[0].inline_data or not response.candidates[0].content.parts[0].inline_data.data:
