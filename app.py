# app.py - VERSÃO FINAL COM ROTEAMENTO INTELIGENTE, ROTAÇÃO DE CHAVES, RECUO EXPONENCIAL E ERROS AMIGÁVEIS
import os
import io
import mimetypes
import struct
...
def generate_audio_endpoint():
    ...
    # --- PARTE 1: ROTAÇÃO DE API KEYS ---
    api_keys_str = os.environ.get("GEMINI_API_KEYS")
    if not api_keys_str:
        error_msg = "ERRO CRÍTICO: Variável de ambiente GEMINI_API_KEYS não encontrada."
        logger.error(error_msg)
        return jsonify({"error_code": "SERVER_CONFIG_ERROR", "message": "Configuração do servidor incompleta."}), 500
    
    # Transforma a string de chaves em uma lista e escolhe uma aleatoriamente
    api_keys = [key.strip() for key in api_keys_str.split(',') if key.strip()]
    if not api_keys:
        return jsonify({"error_code": "SERVER_CONFIG_ERROR", "message": "Nenhuma chave de API válida encontrada na configuração."}), 500
    ...
    # --- PARTE 2: LÓGICA DE RECUO EXPONENCIAL ---
    max_retries = 4
    base_delay = 2
    ...
    for attempt in range(max_retries):
        try:
            ...
            if not audio_data_chunks:
                 return jsonify({"error_code": "NO_AUDIO_DATA", "message": "A API respondeu, mas não retornou dados de áudio."}), 500
            ...
            if attempt == max_retries - 1:
                logger.error("Número máximo de tentativas atingido. Enviando erro de sobrecarga para o cliente.")
                
                # Cria uma resposta de erro padronizada e amigável para o frontend
                custom_error_response = {
                    "error_code": "SERVICE_OVERLOADED",
                    "message": "Nossos sistemas estão com alta demanda no momento. Por favor, aguarde um pouco e tente novamente."
                }
                return jsonify(custom_error_response), 429
            ...
            # Para outros erros, mantemos uma resposta mais genérica
            return jsonify({"error_code": "INTERNAL_SERVER_ERROR", "message": "Ocorreu um erro inesperado no servidor."}), 500
