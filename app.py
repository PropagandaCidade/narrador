...
app = Flask(__name__)
CORS(app, resources={r"/generate-audio": {"origins": "*"}}, supports_credentials=True)
...
def generate_audio_endpoint():
    ...
    try:
        ...
        response = jsonify({"audio_data": audio_b64, "model_used": "pro"})
        ...
        response = jsonify({"error": f"Erro ao contatar a API do Gemini: {e}"})
        response.headers.add("Access-Control-Allow-Origin", "*")
        response.headers.add("Access-Control-Allow-Headers", "Content-Type,Authorization")
        response.headers.add("Access-Control-Allow-Methods", "POST,OPTIONS")
        return response, 500
